...
import com.kyg.stationareayouthhousing.Constants.splitStringToAddress
...
class MainActivityViewModel @Inject constructor(private val repository: Repository) : ViewModel() {
    ...
    private fun crawlingPlan() {
        ...
        for (plan in contentData) {
            ...
            val complexName = element[1].text()
            val address: Address = element[2].text().splitStringToAddress()
            val station = element[3].text()
            val supply = Supply(element[6].text(), element[7].text(), element[8].text())
            val publicDueDate = element[4].text()
            val privateDueDate = element[5].text()
            val tempPlan = Plan(year, complexName, address, station, supply, publicDueDate, privateDueDate)
